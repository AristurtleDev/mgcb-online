...
public class SpriteFontPageModel : PageModel
{
    public class Input
    {

        [DisplayName("Spritefont File")]
        ...
        [DisplayName("Font TTF File")]
        public IFormFile? Font { get; set; } = null;
    ...
    public SpriteFontPageModel(IWebHostEnvironment environment)
    {
    ...
    public async Task<ActionResult> OnPostAsync()
    {
        ...
        if (Form.Font is null) {return Page();}
        ...
        //  Copy the font file to the directory as well
        string fontFilePath = Path.Combine(contentDir, Path.GetFileName(Form.Font.FileName));
        using(FileStream fontFileStream = System.IO.File.Create(fontFilePath))
        {
            await Form.Font.CopyToAsync(fontFileStream);
        }
        ...
        mgcbInfo.ArgumentList.Add($"/importer:FbxImporter");
